...
<doc:template name="xref.xreflabel" xmlns="">
  ...
  <doc:description>
	...
	<doc:template>
		...
		<doc:notes>
			<para>
				The cross-referencing system in the DocBook XSL stylesheets has
				changed a few times, and we are a bit out of date. Within these
				stylesheets, the xrefstyle attribute is currently being used as
				the xref context (or something like that...it's a bit confusing).
			</para>
		</doc:notes>
	</doc:template>
          ...
          <xsl:with-param name="xrefstyle">
            <xsl:choose>
              <xsl:when test="$xrefstyle!=''">
                <!-- note, this prevents a user from enforcing @xrefstyle='': does that matter? -->
                <xsl:value-of select="$xrefstyle"/>
              </xsl:when>
              <xsl:when test="$role!='' and $use.role.as.xrefstyle = 1">
