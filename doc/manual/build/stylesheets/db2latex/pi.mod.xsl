...
<!--############################################################################# 
|	$Id: pi.mod.xsl,v 1.7 2004/12/05 09:27:22 j-devenish Exp $
|- #############################################################################
|	$Author: j-devenish $
+ ############################################################################## -->
...
<xsl:stylesheet
	...
	<doc:template xmlns="">
		<refpurpose> Output arbitrary &LaTeX; text in enumerated lists </refpurpose>
		<doc:description>
			<para>
			...
			output directly by &DB2LaTeX;. This allows the
			inclusion of &latex;-specific instructions that
			will tweak <sgmltag>itemizedlist</sgmltag>,
			<sgmltag>orderedlist</sgmltag> and <sgmltag>variablelist</sgmltag>.
			Unlike the <sgmltag class="pi">latex</sgmltag> and
			<sgmltag class="pi">db2latex</sgmltag> processing instructions,
			the contents of a <sgmltag class="pi">db2latex-custom-list</sgmltag>
			are emitted within the list environment, not preceding it.
