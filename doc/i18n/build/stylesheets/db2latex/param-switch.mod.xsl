...
<xsl:stylesheet
	...
	<doc:param xmlns="">
		...
		<para>One downside is that some 'narrow' tables might fill the entire
		width of the column when <productname>ltxtable</productname> is enabled.</para>
		<warning><para>
		Cells spanning multiple columns may require extra passes with <productname>LaTeX</productname> 
		in order for column widths to 'converge'.
		</para></warning>
		<note><para>
		This is not implemented as true <productname>ltxtable</productname> support, yet.
		It uses <productname>longtable</productname> until we can integrate proper <productname>ltxtable</productname> support.
