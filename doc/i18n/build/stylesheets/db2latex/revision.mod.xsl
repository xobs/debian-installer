...
<xsl:stylesheet
	...
	<doc:template xmlns="">
		...
		<doc:params>
			<variablelist>
				<varlistentry>
					<term>align</term>
					<listitem><simpara>The &LaTeX; cell alignment to use for
					<sgmltag>revremark</sgmltag> and
					<sgmltag>revdescription</sgmltag>.
					Defaults to <literal>l</literal> but you might like to
					try <literal>p{\columnwidth}</literal> for full-width
					word wrapping.</simpara></listitem>
				</varlistentry>
			</variablelist>
		</doc:params>
		<doc:notes>
	...
	<xsl:template match="revision" name="revision">
